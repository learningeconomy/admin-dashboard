...
const FIELDS_TO_DISPLAY = ['earnerName', 'emailAddress', 'actionButton'];

type BatchMembershipListPreviewProps = {
    data: PaginatedDocs<Membership>;
    refetch: (page: number) => Promise<void>;
    readOnly?: boolean;
};

const BatchMembershipListPreview: React.FC<BatchMembershipListPreviewProps> = ({
    data,
    refetch,
    readOnly = false,
}) => {
    const { collections } = useConfig();
    const collection = collections.find(collection => collection.slug === 'membership');
...
export default BatchMembershipListPreview;
