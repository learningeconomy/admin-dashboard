...
const BatchPreviewSubmit = React.forwardRef<HTMLElement, BatchPreviewSubmitProps>(
    function BatchPreviewSubmit({ refetchBatchMemberships, membershipData }, ref) {
        ...
        useEffect(() => {
            if (fields.template.value) {
                fetch(`/api/membership-template/${fields.template.value}`)
                    .then(res => res.json())
                    .then(setTemplate);
            } else {
            ...
            >
                ...
                <Accordion type="single" className="mb-5" defaultValue="credentials">
                    <AccordionItem value="credentials">
                        <AccordionTrigger>Memberships From CSV</AccordionTrigger>
