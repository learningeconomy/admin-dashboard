...
import { Membership } from 'payload/generated-types';
            ...
            return (
                <>
                    ...
                    <p className="text-[--theme-text] text-xl font-medium font-inter mb-15">
                        Provide information about the memberships you’re uploading to this batch to
...
const FormSteps = (props: Props) => {
    ...
    const [membershipData, setMembershipData] = useState<PaginatedDocs<Membership>>();
    ...
    const fetchCsvFields = async () => {
        const response = await fetch('/api/get-batch-fields', {
            method: 'POST',
            body: JSON.stringify({ id, collection: 'membership' }),
    ...
    const fetchBatchMemberships = async (page = 1) => {
        fetchCsvFields();
        const response = await fetch('/api/get-batch-credentials', {
            method: 'POST',
            body: JSON.stringify({ batchId: id, page, collection: 'membership' }),
            headers: { 'Content-type': 'application/json; charset=UTF-8' },
        });
    ...
    const duplicate = async () => {
        const result = await fetch(`/api/membership-batch/${id}/duplicate`, { method: 'POST' });
        ...
        <>
            <section className="flex-grow w-full snap-mandatory snap-x overflow-x-hidden flex-shrink pt-12 flex md:pt-0">
                ...
                <UploadCSV
                    ...
                    refetchBatchMemberships={fetchBatchMemberships}
...
const RenderMembershipBatchFlowFields: React.FC<Props> = props => {
...
export default RenderMembershipBatchFlowFields;